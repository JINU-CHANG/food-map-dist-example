...
var options = { //지도를 생성할 때 필요한 기본 옵션
	center: new kakao.maps.LatLng(37.5659, 126.9851), //지도의 중심좌표.
...
async function getDataSet(category) {
  ...
  const dataSet = await axios({
    method: "get",
    url: `http://3.34.243.198:3000/restaurants?category=${qs}`,
...
function getContent(data) {
  ...
  return `
  <div id="infowindow">
    <div class="infowindow-img-container"></div>
    <img src="https://img.youtube.com/vi/${finUrl}/mqdefault.jpg" class="imfowindow-img"/>
    <div class="infowindow-body">
      <h5 class="infowindow-title">${data.title}</h5>
      <p class="infowindow-address">${data.address}</p>
      <a href="${data.videoUsrl}" class="infowindow-btn" target="_blank">영상이동</a>
    </div>
  </div>`;
}
